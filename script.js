 ...
 */
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbw2pXx4-sVQjyUlhrKyCSlqo_ljzabiDsXE3pc0pa747gbxswO7-ZMe1J4TolisFKGB/exec';
...
(function () {
  ...
  // v43: 첫 본인확인 화면에서는 조직도 전체를 불러오지 않는다.
  // 모바일에서 사번 조회와 조직도 조회가 동시에 걸리면 시간 초과가 발생할 수 있어,
  // 조직도는 신규 선임 매장 추가가 필요한 순간에만 지연 로딩한다.
  var v43OrgTreePromise = null;
  function loadOrganizationTreeActual() {
    if (Object.keys(orgTree || {}).length) return Promise.resolve(orgTree);
    if (v43OrgTreePromise) return v43OrgTreePromise;
    if (!validateAppsScriptUrl(false)) {
      return Promise.reject(new Error('Apps Script URL을 먼저 script.js에 입력해주세요.'));
    }
    try {
      setGlobalOrgMessage && setGlobalOrgMessage('pending', '매장 목록을 불러오는 중입니다.');
      setOrgMessage && setOrgMessage('pending', '매장 목록을 불러오는 중입니다.');
    } catch(e) {}
    v43OrgTreePromise = jsonpRequest({ mode: 'org' }, 30000)
      .then(function (data) {
        if (!data || !data.success) throw new Error(data && data.message ? data.message : '매장 목록 불러오기 실패');
        orgTree = data.tree || {};
        ...
        try {
          setGlobalOrgMessage && setGlobalOrgMessage('success', '매장 목록 준비 완료');
          setOrgMessage && setOrgMessage('success', '매장 목록 준비 완료');
        } catch(e) {}
        return orgTree;
      })
      .catch(function (error) {
        v43OrgTreePromise = null;
        console.error(error);
        try {
          setGlobalOrgMessage && setGlobalOrgMessage('error', '매장 목록을 준비하지 못했습니다. 다시 시도해주세요.');
          setOrgMessage && setOrgMessage('error', '매장 목록을 준비하지 못했습니다.');
        } catch(e) {}
        throw error;
      });
    return v43OrgTreePromise;
  }
  window.ensureV43OrgTreeLoaded = loadOrganizationTreeActual;
  window.loadOrganizationTree = function () {
    // 초기 접속 시 자동 호출되는 기존 코드를 막기 위한 no-op.
    // 실제 조직도는 window.ensureV43OrgTreeLoaded()로 필요한 순간만 불러온다.
...
/* =========================================================
   v43 모바일 조회 안정화 패치
   - 첫 화면에서는 조직도 전체 로딩 금지
   - 신규 선임 매장 추가 시에만 조직도 지연 로딩
   - 선임현황 로딩창 잔류 방지
   ========================================================= */
(function () {
  function orgTreeReady() {
    try { return !!(orgTree && Object.keys(orgTree).length); } catch (e) { return false; }
  }

  document.addEventListener('click', function (event) {
    var addBtn = event.target && event.target.closest ? event.target.closest('#addAppointmentStoreBtn') : null;
    if (!addBtn) return;
    if (orgTreeReady()) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    if (typeof showLoading === 'function') {
      showLoading(true, '신규 선임에 필요한 매장 목록을 불러오는 중입니다.', '매장 목록 준비 중');
    }
    var loader = (typeof window.ensureV43OrgTreeLoaded === 'function') ? window.ensureV43OrgTreeLoaded() : Promise.resolve();
    loader.then(function () {
      if (typeof showLoading === 'function') showLoading(false);
      setTimeout(function () { addBtn.click(); }, 30);
    }).catch(function (err) {
      if (typeof showLoading === 'function') showLoading(false);
      var msg = document.getElementById('appointmentResultMessage');
      if (msg) {
        msg.className = 'result error';
        msg.innerHTML = '매장 목록을 불러오지 못했습니다.<br>' + (err && err.message ? err.message : '잠시 후 다시 시도해주세요.');
      }
    });
  }, true);

  // 선임/해임 화면 진입 시 기존 전체 로딩창이 남아 있으면 사용자가 멈춘 것으로 보이므로 자동 해제한다.
  document.addEventListener('click', function (event) {
    var btn = event.target && event.target.closest ? event.target.closest('#showAppointmentModuleBtn, [data-nav-module="appointment"], #loginGoAppointmentBtn') : null;
    if (!btn) return;
